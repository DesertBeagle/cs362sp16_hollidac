...
int getWinners(int players[4], struct gameState *state);


int cardEffect(int, int, int, int, struct gameState *, int, int*);
# 2 "src.c" 2
# 1 "dominion_helpers.h" 1
...
who = state -> whoseTurn ;
if ( state -> numBuys < 1 ) {
if ( 0 ) printf ( "You do not have any buys left\n"
 ) ;

return - 1 ;
}
else if ( supplyCount ( supplyPos , state ) < 1 ) {
if ( 0 ) printf ( "There are not any of that type of card left\n"
 ) ;

return - 1 ;
}
else if ( state -> coins < getCost ( supplyPos ) ) {
if ( 0 ) printf ( "You do not have enough money to buy that. You have %d coins.\n"
 , state -> coins ) ;
...
if ( 0 ) printf ( "None of that card left, sorry!\n"
 ) ;
...
if ( 0 ) printf ( "Player %d reveals card number: %d\n"
